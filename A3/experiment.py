...
class Settings(BaseModel):
    b: int = Field(default=3, description="branching factor")

    epsilon: float = Field(
        default=0.1, description="chance an exploratory action will be preformed"
    )

    num_states: int = Field(default=10000, description="Total number of states")

    num_actions: int = Field(
        default=2, description="total actions that can be taken at a state"
    )

    sample_task: int = Field(
        default=200, description="number of episodic task preformed"
    )

    max_steps: int = Field(
        default=20000, description="how many expected updates to take"
    )

    interval: int = Field(
        default=200, description="every steps evaulate using a greedy policy"
    )

    runs: int = Field(
        default=1000, description="Number of runs for the policy to average over"
    )
...
class EnviornmentModel(BaseModel):
    ...
    starting_state: int = Field(
        default=0, description="episode always starts in this state"
    )

    terminal_state: int = Field(
        default=5, description="if this state is chosen the episode ends"
    )

    state_action_branch_matrix: NDArray

    terminate_episode: bool = Field(
        default=False, description="Have we reached the terminal state"
    )
...
def main():
    ...
    envs = [
        EnviornmentModel(
            ...
            rewards=np.random.normal(
                loc=0, scale=1, size=(s.num_states, s.num_actions, s.b)
            ),
            terminal_state=np.random.randint(s.num_states),
            state_action_branch_matrix=np.random.randint(
                s.num_states, size=(s.num_states, s.num_actions, s.b)
            ),
        )
        for _ in range(s.sample_task)
    ]
    ...
    for env in trange(len(envs)):
        ...
        t_uniform = TrajectorySampling(
            config=s, env=envs[env], Q=np.zeros(shape=(s.num_states, s.num_actions))
        )
    ...
    plt.title(
        f"{s.num_states} States averaged over {s.sample_task} runs {s.b} Branches"
    )
    plt.legend()
    plt.show()
